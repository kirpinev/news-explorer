...
export default class Header extends BaseComponent {
  ...
  setLoggedHeaderNavigation(userName) {
    this._setLoggedHeaderNavigation();

    this._setButtonLoginText(userName);
  }

  setUnloggedHeaderNavigation() {
    this._setUnloggedHeaderNavigation();
  }
  ...
  _logout() {
    ...
    mainApi
      ...
      .then(res => {
        if (res.status === '200') {
          this.setUnloggedHeaderNavigation();
  ...
  setHandlers() {
    const { headerToggle, popupNavLinks, buttonLogin, headerMain, buttonAuth } = this.elements;
