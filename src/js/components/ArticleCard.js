...
export default class ArticleCard extends BaseComponent {
  ...
  create(article, searchKeyword, articlesContainer) {
    const { checkImageUrl, dates, deleteListTags, MONTHS } = this._dependencies;

    const { text, date, source, title, link, image, keyword, id = ' ' } = this._getArticlesProps(
      articlesContainer,
      article,
      searchKeyword
    );
    ...
    return `<div class="article" id="${id}">
        ...
        <p class="article__search-keyword" style="display: ${this._setKeywordVisibility(articlesContainer)};">${keyword}</p>
          <p class="article__action-description">${this._setActionDescription(articlesContainer)}</p>
          <button type="button" class="article__action-button ${this._setActionButton(articlesContainer)}"></button>
        </div>
  ...
  // JSON поля mainApi и newsApi отличаются, эта функция упрощает работу с ними
  ...
  toggleActionDescription(event) {
    ...
    const { actionBlockOpened } = this.props;
    ...
    if ((event.target.classList.contains(notAddedIcon) && !userName) || event.target.classList.contains(deleteIcon)) {
      event.target.parentNode.querySelector(actionBlock).classList.toggle(actionBlockOpened);
    }
  }
