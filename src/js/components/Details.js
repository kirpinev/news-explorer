...
export default class Details extends BaseComponent {
  ...
  _fillKeywordContainer(articles) {
    ...
    if (keywordsArr.length <= 0) {
      resultString = `<p class="details__keywords">По ключевым словам: ничего не найдено :(</p>`;
    } else if (keywordsArr.length === 1) {
      resultString = `<p class="details__keywords">По ключевым словам: <strong class="details__accent">${keywordsArr[0][0]}</strong></p>`;
    } else if (keywordsArr.length === 2) {
      resultString = `<p class="details__keywords">По ключевым словам: <strong class="details__accent">${keywordsArr[0][0]}</strong> и <strong class="details__accent">${keywordsArr[1][0]}</strong></p>`;
    } else if (keywordsArr.length === 3) {
      resultString = `<p class="details__keywords">По ключевым словам: <strong class="details__accent">${keywordsArr[0][0]}</strong>, <strong class="details__accent">${keywordsArr[1][0]}</strong> и <strong class="details__accent">${keywordsArr[2][0]}</strong></p>`;
    } else {
      resultString = `<p class="details__keywords">По ключевым словам: <strong class="details__accent">${keywordsArr[0][0]}</strong>, <strong class="details__accent">${keywordsArr[1][0]}</strong> и <strong class="details__accent">${keywordsArr.length - 2} другим</strong></p>`;
    }
